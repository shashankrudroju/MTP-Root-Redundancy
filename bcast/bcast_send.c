...
// Values set for ARP request. (plus an extra ID - Peter Willis)
#include <stdio.h>
#include <stdlib.h>
...
struct _arp_hdr {
  ...
  uint8_t target_ip[4];
  int frameID; //added for clarity in testing MTP
};

// Define some constants.
#define ETH_HDRLEN 14      // Ethernet header length
#define IP4_HDRLEN 20      // IPv4 header length
#define ARP_HDRLEN 29      // ARP header length, was 28, now it's 29
...
int main (int argc, char **argv)
{
  ...
  /*
  //added to make each frame unique
  //char* fourthOctet;
  //fourthOctet = allocate_strmem(3);
  //printf("Please enter a valid number 1-255: ");
  //scanf("%s", fourthOctet);
  //printf("you entered: %s\n", fourthOctet);

  char sourceIP[16];
  printf("Please enter a valid number 1-255: ");
    if (fgets(sourceIP, sizeof sourceIP, stdin))
    {
       printf("you entered: %s\n", sourceIP);
    }
    */
  ...
  // ID FOR MTP TESTING - Peter W.
  arphdr.frameID = customID;
